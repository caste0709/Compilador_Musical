...
struct NodoArbol {
    ...
    NodoArbol(const string& etiqueta) : etiqueta(etiqueta) {}

    void agregarHijo(const NodoArbol& hijo) {
...
public:
    ...
    NodoArbol arbolSintactico = NodoArbol("Analizador Sintactico");

    void parse() {
        NodoArbol declaraciones("Declaraciones");
        while (pos < tokens.size()) {
            if (tokens[pos].tipo == "Tempo" || tokens[pos].tipo == "Compas" ||
                ...
                declaraciones.agregarHijo(NodoArbol(tokens[pos].tipo + " = " + tokens[pos].lexema));
                ...
                NodoArbol def("Definicion: " + tokens[pos].lexema.substr(0, tokens[pos].lexema.size()-1));
                match("Definicion");
                NodoArbol bloqueNodo = bloque();  // modifica bloque() para devolver NodoArbol
                def.agregarHijo(bloqueNodo);
                arbolSintactico.agregarHijo(def);
            } else if (tokens[pos].tipo == "Repeat") {
                NodoArbol rep("Repeticion: " + tokens[pos].lexema);
                arbolSintactico.agregarHijo(rep);
                match("Repeat");
            } else {
                lanzarError("Tokens inesperados al final del archivo");
            }
        }
        arbolSintactico.agregarHijo(declaraciones);
    }
...
private:
    ...
    NodoArbol bloque() {
        ...
        while (pos < tokens.size() && tokens[pos].tipo != "BloqueFin") {
            NodoArbol compas = compasRitmico();
            nodoBloque.agregarHijo(compas);
    ...
    NodoArbol compasRitmico() {
        NodoArbol nodoCompas("Compas Ritmico");

        while (pos < tokens.size() && tokens[pos].tipo == "NotaFigura") {
            string nota = tokens[pos].lexema;
            NodoArbol nodoNota("Nota -> " + nota);
            smatch m;
            regex r("\\(([^,]+),([^\\)]+)\\)");
            if (regex_match(nota, m, r)) {
                ...
                if (tienePuntillo) {
                    dur *= 1.5;
                    nodoNota.agregarHijo(NodoArbol("Decorador: ."));
                }
            ...
            match("NotaFigura");
            match("DecoradorPuntillo");
            nodoCompas.agregarHijo(nodoNota);
        }
...
int main() {
    ...
    try {
        ...
        parser.parse();
        parser.arbolSintactico.imprimir();
        ...
        for (size_t i = 0; i < pistas.size(); ++i) {
            ...
            sample << Instrument(0, i)(instrumento); // <-- Cambia el instrumento aqui
